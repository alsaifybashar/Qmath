...
CREATE TABLE `attempt_logs` (
	...
	`student_answer_raw` text,
	`feedback_code` text,
	`partial_score` real,
	`confidence_rating` integer,
	`symbolically_checked` integer,
	`hints_used` integer,
	`help_level_reached` integer,
	`session_id` text,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `course_assignments_professor_course_idx` ON `course_assignments` (`professor_id`,`course_id`);--> statement-breakpoint
...
CREATE TABLE `learning_events` (
	...
	FOREIGN KEY (`question_id`) REFERENCES `questions`(`id`) ON UPDATE no action ON DELETE set null,
	FOREIGN KEY (`topic_id`) REFERENCES `topics`(`id`) ON UPDATE no action ON DELETE set null,
	FOREIGN KEY (`step_id`) REFERENCES `question_steps`(`id`) ON UPDATE no action ON DELETE set null
);
--> statement-breakpoint
CREATE INDEX `learning_events_user_created_idx` ON `learning_events` (`user_id`,`created_at`);--> statement-breakpoint
CREATE INDEX `learning_events_type_idx` ON `learning_events` (`event_type`);--> statement-breakpoint
...
CREATE TABLE `flashcard_card_state` (
